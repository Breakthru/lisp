...
(defun sost1 (nome lambda lista)
	(cond ((atom lista) lista)
		  (t (sost nome lambda lista))))

(defun label (nome le &rest valori) (eval (append `( ,`(lambda ,(cadr le) ;gli argomenti
				  ,(sost nome le (caddr le) ) )) (mapcar 'quota valori))
)
)

(defun quota (a) `',a)

(defun clabel (nome le &rest valori)  (append `( ,`(lambda ,(cadr le) ;gli argomenti
				  ,(sost nome le (caddr le) ) )) (mapcar 'quota valori))
)

(defmacro m-label (nome le &rest valori)
(append `(label ',nome ',le) (mapcar 'quota valori))
)
...
;(label 'p '(lambda (x) (cond ((zerop x) 0) (T  (+ x (p (- x 1)) )))) 8)

(defmacro m-eval (espressione ambiente)
`(LABEL 'EVALM
 '(LAMBDA (E A)
   (COND
    ((ATOM E)
